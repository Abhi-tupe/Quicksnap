...
# 💬 Chat Assistant Tab
    ...
    st.markdown("Talk to me naturally — I can create and edit images using Bria API.")
    ...
    if user_input:
        ...
        if task == "generate":
            ...
            try:
                ...
                # Handle all possible Bria API response formats
                img_url = None
                if isinstance(result, dict):
                    if "result_url" in result:
                ...
                if img_url:
                    ...
                    if img_data:
                        st.session_state.chat_history.append({"role": "assistant", "type": "image", "content": img_data})
                        with st.chat_message("assistant"):
                            st.image(img_data, caption="Generated Image", use_column_width=True)
                else:
                    with st.chat_message("assistant"):
                        st.markdown("❌ I couldn't generate the image.")

            except Exception as e:
                with st.chat_message("assistant"):
                    st.markdown(f"⚠️ Error: {str(e)}")

        elif task == "shadow" and st.session_state.last_image:
            with st.chat_message("assistant"):
                st.markdown("🖌️ Adding shadow to last image...")
            try:
                img_data = download_image(st.session_state.last_image)
                if img_data:
                    result = add_shadow(
                        api_key=st.session_state.api_key,
                        image_data=img_data,
                    ...
                    if img_url:
                        ...
                        if img_data:
                            st.session_state.chat_history.append({"role": "assistant", "type": "image", "content": img_data})
                            with st.chat_message("assistant"):
                                st.image(img_data, caption="Image with Shadow", use_column_width=True)

            except Exception as e:
                with st.chat_message("assistant"):
                    st.markdown(f"⚠️ Error: {str(e)}")

        elif task == "lifestyle" and st.session_state.last_image:
            with st.chat_message("assistant"):
                ...
                if img_data:
                    ...
                    if img_url:
                        ...
                        if img_data:
                            st.session_state.chat_history.append({"role": "assistant", "type": "image", "content": img_data})
                            with st.chat_message("assistant"):
                                st.image(img_data, caption="Lifestyle Shot", use_column_width=True)
            ...
            with st.chat_message("assistant"):
                st.markdown("💬 I can chat with you, but for now I only handle image generation, shadows, and lifestyle shots.")
